...
- 1000~ 부터 일반 사용자에게 id를 줌=> 파일의 종류|크기|소유자, 파일 변경시간, 파일명 등등 파일의 상세정보와 [Data] 주소(index)가 담김.
...
- shadow의 $(달러)는 암호화가 설정되었음을 의미함.ㄴ ps -ef 이거 모르면 리눅스 하지마.
- -f: 프로세스 상세정보
- -e: 전체 프로세스 목록 출력
